...
def register():
    ...
    if register_form.validate_on_submit():
        if User.query.filter_by(email=register_form.email.first()):
            flash('Sorry this email already exists, try logging in!')
            return redirect(url_for('login'))
       
        hash_and_salt = generate_password_hash(register_form.password.data, method='pbkdf2:sha256', salt_length=8)
        new_user = User(
